...
class BLIP_Pretrain(nn.Module):
    ...
    def forward(self, image, caption, alpha):
        ...
        image_embeds = self.visual_encoder(image)  # (75, 3, 224, 224) -> (75, 197, 768)
        ...
        output_pos = self.text_encoder(
            ...
            return_dict=True,
        )  # last_hidden_layer (75, 30, 768)

        # calculated weights based on similarity
        with torch.no_grad():
            weights_t2i = F.softmax(sim_t2i[:, :bs], dim=1) + 1e-4  # (75, 75)
        ...
        image_embeds_neg = torch.stack(image_embeds_neg, dim=0)  # (75, 197, 768)
        ...
        text_ids_neg = torch.stack(text_ids_neg, dim=0)  # (75, 30)
        text_atts_neg = torch.stack(text_atts_neg, dim=0)  # (75, 30)

        text_ids_all = torch.cat([encoder_input_ids, text_ids_neg], dim=0)  # (150, 30)
        text_atts_all = torch.cat(
            [text.attention_mask, text_atts_neg], dim=0
        )  # (150, 30)

        image_embeds_all = torch.cat(
            [image_embeds_neg, image_embeds], dim=0
        )  # (150, 197, 768)
        ...
        vl_embeddings = torch.cat(
            ...
            dim=0,
        )  # (225, 768) from (75, 768) pos + (150, 768) negative
